...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // (proj2): implement
        int order = metadata.getOrder();
        while (data.hasNext() && keys.size() < order * 2 * fillFactor + 1) {
            Pair<DataBox, RecordId> p = data.next();
            keys.add(keys.size(), p.getFirst());
            rids.add(rids.size(), p.getSecond());
        }
        ...
        if (data.hasNext()) {
            ...
            rightRids.add(0, rids.get(rids.size() - 1));
    ...
    public Pair<DataBox, Long> split() {
        ...
        sync();
        return new Pair<>(rightKeys.get(0), rightSibling.get());
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        for (int i = 0; i < n; ++i) {
            ...
            rids.add(new RecordId(buf.getLong(), buf.getShort()));
        }

        return new LeafNode(metadata, bufferManager, page, keys, rids, Optional.of(sibNum), treeContext);
    }
